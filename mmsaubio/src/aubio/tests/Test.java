...
public class Test {
    ...
    public static void main(String[] args) {
    	...
    	try {
    		System.loadLibrary("jaubio");
    	} catch (UnsatisfiedLinkError e) {
    		System.out.println("Failed to load the library \"jaubio\"");
		...
		for (int i = 0; i < buffer.length; i++) {
			buffer[i] = 0.8 * Math.sin(2 * Math.PI * fs * t);
			t = t + step;
		}
		...
		for (int i = 0; i < (int)(fs * duration); i++) {
			aubiowrapper.fvec_write_sample(in, (float)i, 0, i);
